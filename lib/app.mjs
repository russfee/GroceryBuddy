...
export async function handleApiRequest(req, res, pathname) {
  try {
    ...
    if (route === "/api/file" && req.method === "POST") {
      ...
      await writeManagedFile(body.name, String(body.content ?? ""));
    ...
    if (route === "/api/week" && req.method === "POST") {
      ...
      await createWeekFile(fileName, content);
...
function githubStore() {
  ...
  return {
    ...
    async write(name, content, message) {
      ...
      await putGithubContent(config, name, content, message, existing?.sha);
    },
    async create(name, content, message) {
      ...
      await putGithubContent(config, name, content, message);
    }
  };
}
