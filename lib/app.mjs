...
export async function handleApiRequest(req, res, pathname) {
  try {
    ...
    if (route === "/api/capture" && req.method === "POST") {
      const body = await readCaptureBody(req);
      const item = normalizeCaptureItem(body.item);
      if (!item) {
        return sendJson(res, { error: "Missing grocery item" }, 400);
      }
      const result = await captureGroceryItem(item, body.source);
    ...
    if (route === "/api/finish-week" && req.method === "POST") {
      const body = await readJson(req);
      const result = await finishWeek(body.title, body.date);
      return sendJson(res, result);
    }

    if (route === "/api/order-placed" && req.method === "POST") {
      const body = await readJson(req);
      if (body.confirmed !== true) {
        return sendJson(res, { error: "Confirm that the grocery order was placed before clearing captures" }, 400);
      }
      const result = await finishWeek(body.title, body.date);
      return sendJson(res, { ...result, orderPlaced: true });
    }
...
async function upsertWeekFile(fileName, content) {
  return createStore().write(`weeks/${fileName}`, content, `Archive grocery week ${fileName.replace(".md", "")}`);
}

async function captureGroceryItem(item, source) {
  const store = createStore();
  const current = await store.read("WeeklyAddOns.md");
  const content = appendCapture(current, item, source);
  const sync = await store.write("WeeklyAddOns.md", content, "Capture grocery item");
  return { ok: true, item, file: "WeeklyAddOns.md", sync };
}

async function finishWeek(title, date) {
  const store = createStore();
  const fileName = safeWeekName(date);
  const files = await readManagedFiles();
  const snapshotContent = buildWeekContent(title, date, files);
  const snapshotSync = await upsertWeekFile(fileName, snapshotContent);

  const addOns = files.files.find((file) => file.name === "WeeklyAddOns.md")?.content || "";
  const clearedContent = clearCaptureSections(addOns);
  const clearSync = clearedContent === addOns
    ? { mode: storageMode(), status: "unchanged" }
    : await store.write("WeeklyAddOns.md", clearedContent, "Clear grocery captures");

  return {
    ok: true,
    file: `weeks/${fileName}`,
    cleared: clearedContent !== addOns,
    snapshotSync,
    clearSync
  };
}
...
export function appendCapture(content, item, source) {
  const label = normalizeCaptureSource(source);
  const heading = `${label} captures:`;
  const bullet = `- ${item}`;
  const base = String(content || DEFAULTS["WeeklyAddOns.md"]).trimEnd();
  const headingPattern = new RegExp(`(^|\\n)${escapeRegExp(heading)}\\n`);

  if (headingPattern.test(base)) {
    return `${base}\n${bullet}\n`;
  }
...
export function clearCaptureSections(content) {
  const lines = String(content || DEFAULTS["WeeklyAddOns.md"]).split(/\r?\n/);
  const kept = [];

  for (let index = 0; index < lines.length; index += 1) {
    const line = lines[index];
    if (isCaptureHeading(line)) {
      index += 1;
...
function isCaptureHeading(line) {
  return /^[A-Za-z][A-Za-z0-9 -]{0,39} captures:$/.test(String(line || "").trim());
}
...
async function readCaptureBody(req) {
  const text = await readBodyText(req);
  ...
  if (contentType.includes("application/x-www-form-urlencoded")) {
    ...
    return {
      item: params.get("item") || params.get("text") || "",
